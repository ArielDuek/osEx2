/**
 * We edit the program dining_philosophers.c never get into a deadlock condition.
 *
 * Unlike the situation where everyone first takes the left fork and then Deadlock is created,
 * this time everyone tries to take the right fork first except one philosopher (the fifth philosopher)
 * who tries to take the left one first (using the condition - "if (phnum > RIGHT)" ),
 * ensuring that there will always be a philosopher who can pick up the The extra fork and start eating.
 *
 * And so Deadlock was NEVER created.
 */
