...
void test(int phnum)
{
    if (state[phnum] == HUNGRY || state[phnum] == TAKE_RIGHT || state[phnum] == TAKE_LEFT || state[phnum] == PUT){
        if ((state[LEFT] != EATING && state[LEFT] != TAKE_RIGHT && state[phnum] != TAKE_LEFT && state[LEFT] != PUT) || (state[phnum] == PUT && state[LEFT] == PUT)) {
            takeLeft(phnum);
            sem_post(&S[phnum]);
        }
        else if ((state[RIGHT] != EATING && state[RIGHT] != TAKE_LEFT && state[phnum] != TAKE_RIGHT && state[RIGHT] != PUT) || (state[phnum] == PUT && state[RIGHT] == PUT)) {
            takeRight(phnum);
            sem_post(&S[phnum]);
        }
        else {
